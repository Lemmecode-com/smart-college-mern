...
export default function SecurityAudit() {
  ...
  const [filters, setFilters] = useState({
    ...
    startDate: '',
    endDate: '',
    collegeId: ''  // For Super Admin to filter by college
  });
  const [currentPage, setCurrentPage] = useState(1);
  const [pagination, setPagination] = useState(null);
  const [colleges, setColleges] = useState([]);  // List of colleges for filter

  useEffect(() => {
    fetchLogs();
    fetchStats();
    fetchColleges();  // Fetch colleges for Super Admin filter
  }, [filters, currentPage]);
  ...
  const fetchLogs = async () => {
    try {
      ...
      console.log('First log sample:', res.data[0]);
      ...
      if (logsData && logsData.length > 0) {
        console.log('First log collegeId:', logsData[0].collegeId);
        console.log('First log collegeId.name:', logsData[0].collegeId?.name);
      }
  ...
  const fetchColleges = async () => {
    try {
      // Use Super Admin college list endpoint
      const res = await api.get('/super-admin/colleges-list');
      if (res.data?.data) {
        setColleges(res.data.data);
      }
    } catch (error) {
      console.error('Failed to fetch colleges:', error);
      // If fetch fails, leave colleges array empty - filter will still work
    }
  };
  ...
  return (
    <div className="security-audit-page" style={{ padding: '20px' }}>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2>🔒 Security Audit Dashboard (Super Admin)</h2>
                    ...
                    {logs && logs.length > 0 ? (
                      logs.map(log => (
                        <tr key={log._id}>
                          ...
                          <td>
                            {log.collegeId?.name || <span className="text-muted">N/A</span>}
                          </td>
                          <td>

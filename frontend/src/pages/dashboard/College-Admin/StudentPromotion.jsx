...
//       console.log("Promotion data:", { remarks: promotionRemarks, overrideFeeCheck });
...
//                   <span className="fee-value">₹{selectedStudent.fee?.totalFee || selectedStudent.totalFee || 0}</span>
//                 </div>
//                 <div className="fee-row">
//                   <span className="fee-label">Paid Amount:</span>
//                   <span className={`fee-value ${
//                     (selectedStudent.fee?.paidAmount || 0) >= (selectedStudent.fee?.totalFee || selectedStudent.totalFee || 0)
//                       ? "text-success"
//                       : ""
//                   }`}>₹{selectedStudent.fee?.paidAmount || selectedStudent.paidAmount || 0}</span>
//                 </div>
...
export default function StudentPromotion() {
  ...
  const handlePromoteStudent = async () => {
    try {
      ...
      const currentYear = getAcademicYear(selectedStudent.currentSemester, selectedStudent.admissionYear);
      const nextYear = getAcademicYear(selectedStudent.currentSemester + 1, selectedStudent.admissionYear);
                  ...
                  {promotionHistory.map((record) => (
                    <tr key={record._id}>
                      ...
                      <td>
                        <div>
                          <span className="text-muted">
                            {record.fromAcademicYear || `Sem ${record.fromSemester}`}
                          </span>
                          <span className="mx-2">→</span>
                          <span className="text-primary fw-bold">
                            {record.toAcademicYear || `Sem ${record.toSemester}`}
                          </span>
                        </div>
                    ...
                    {paginatedStudents.map((student) => (
                      <tr key={student._id}>
                        ...
                        <td>
                          <div>
                            <span className="badge badge-info">
                              {getAcademicYear(student.currentSemester, student.admissionYear)}
                            </span>
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              ...
              <div className="student-info-card">
                ...
                <div className="promotion-info">
                  <span className="badge badge-info">
                    {getAcademicYear(selectedStudent.currentSemester, selectedStudent.admissionYear)} 
                    (Sem {selectedStudent.currentSemester}) 
                    → 
                    {getAcademicYear(selectedStudent.currentSemester + 1, selectedStudent.admissionYear)} 

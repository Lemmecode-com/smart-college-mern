...
export default function TimetableList() {
  ...
  const publishTimetable = async (id) => {
    if (!window.confirm("Are you sure you want to publish this timetable? Students will be able to view it.")) return;
        ...
        <div style={{ maxWidth: '100%', margin: '0 auto' }}>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <span style={{ color: BRAND_COLORS.primary.main, fontWeight: 600, fontSize: '0.9rem' }}>Timetables</span>
            </div>
          </motion.div>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '2rem', flexWrap: 'wrap' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <span style={{ color: '#4a5568', fontWeight: 500 }}>
                    Published: <strong>{timetables.filter(t => t.status === 'PUBLISHED').length}</strong>
                  </span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <FaEdit style={{ color: BRAND_COLORS.warning.main }} />
                  <span style={{ color: '#4a5568', fontWeight: 500 }}>
                    Draft: <strong>{timetables.filter(t => t.status === 'DRAFT').length}</strong>
                  </span>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          >
            {timetables.length > 0 ? (
              ...
              <EmptyState onRefresh={fetchTimetables} onCreate={() => navigate('/timetable/create')} />
            )}
          </motion.div>
        </div>
      </motion.div>
...
/* ================= TIMETABLE CARD ================= */
function TimetableCard({ timetable, isTeacher, onPublish, onDelete, deletingId, publishingId, delay = 0 }) {
  const navigate = useNavigate();
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
              <FaInfoCircle size={16} style={{ color: BRAND_COLORS.primary.main }} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.75rem', fontSize: '0.9rem' }}>
              <div>
                ...
                <div style={{ fontWeight: 500, color: '#1e293b', marginTop: '0.25rem' }}>
              ...
              <>
                <motion.button
                  ...
                  style={{
                    ...
                    borderRadius: '10px',
                    border: 'none',
                    backgroundColor: publishingId === timetable._id ? '#94a3b8' : BRAND_COLORS.success.main,
                    color: 'white',
                    fontSize: '0.95rem',
                    fontWeight: 600,
                    cursor: publishingId === timetable._id ? 'not-allowed' : 'pointer',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    gap: '0.5rem',
                    transition: 'all 0.3s ease',
                    boxShadow: publishingId === timetable._id ? 'none' : '0 4px 10px rgba(40, 167, 69, 0.3)'
                  }}
                >
                ...
                <motion.button
                  ...
                  style={{
                    ...
                    borderRadius: '10px',
                    border: 'none',
                    backgroundColor: deletingId === timetable._id ? '#94a3b8' : BRAND_COLORS.danger.main,
                    color: 'white',
                    fontSize: '0.95rem',
                    fontWeight: 600,
                    cursor: deletingId === timetable._id ? 'not-allowed' : 'pointer',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    transition: 'all 0.3s ease',
                    boxShadow: deletingId === timetable._id ? 'none' : '0 4px 10px rgba(220, 53, 69, 0.3)'
                  }}
                >
  ...
  return (
    <div style={{
      ...
      <p style={{ 
        ...
        You haven't created any timetables yet. Create your first timetable to start managing your academic schedule.
      </p>

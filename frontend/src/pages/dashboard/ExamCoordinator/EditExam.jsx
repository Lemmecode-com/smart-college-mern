...
/* =========================================================
   Internal CSS — identical navy/cyan token set and class
   names as CreateExam's .exam-form, so Create and Edit
   look and feel exactly the same. Scoped under .exam-form
   so it never leaks into other pages.
   ========================================================= */
const formStyles = `
.exam-form {
  --edx-bg: #f4f7fa;
...
.exam-form .fallback-wrap { max-width: 560px; margin: 2rem auto; }
...
export default function EditExam() {
  ...
  useEffect(() => {
    const fetchExam = async () => {
      try {
        ...
        setExam(examData);
        const courseDeptId =
          examData.course_id?.department_id
            ? String(
                examData.course_id.department_id._id ||
                  examData.course_id.department_id,
              )
            : "";
        setFormData({
          ...
          name: examData.name || "",
          course_id: examData.course_id?._id || examData.course_id || "",
          semester: examData.semester || "",
          academicYear: examData.academicYear || "",
          subjects: examData.subjects?.map((s) => s.subject?._id || s.subject) || [],
        });
      } catch (err) {
        const statusCode = err.response?.status;
        const errorCode = err.response?.data?.code;
        const backendMessage = err.response?.data?.message;
        setError({ message: backendMessage || "Failed to load exam details.", statusCode, errorCode });
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      logger.error("Error updating exam:", statusCode, errorCode);

      if (AUTH_ERROR_CODES.has(errorCode)) {
        setError({ message: backendMessage, statusCode, errorCode, isAuthError: true });
      } else {
        setError(backendMessage || "Failed to update exam. Please try again.");
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="exam-form container-fluid p-4">
        ...
        <div className="success-screen">
          ...
          <div className="alert-edx alert-edx-success" style={{ justifyContent: "center" }}>
            <FaCheckCircle />
            Exam updated successfully! Redirecting...
          </div>
        </div>
      </div>
    );
        ...
        <div className="col-lg-8">
          ...
          >
            <div className="exam-card-header">
              ...
              <h4 className="exam-card-title">Edit Exam</h4>
            </div>
            <div className="exam-card-body">
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="field-group">
                  <label className="field-label">
                    <FaClock className="field-label-icon" />
                    Academic Year *
                  </label>
                  <input
                    type="text"

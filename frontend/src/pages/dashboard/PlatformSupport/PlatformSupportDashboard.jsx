  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [
        ...
        api.get("/platform-support/health/history?hours=24"),
        api.get("/platform-support/health/integrations"),
        api.get("/platform-support/colleges/health"),
        api.get("/platform-support/tickets?limit=5"),
        api.get("/platform-support/tickets/stats"),
        api.get("/platform-support/error-stats?hours=24"),
      ]);

      // Unwrap data (axios interceptor extracts)
      setLatestHealth(healthRes.data?.health || null);
      setHealthHistory(historyRes.data?.history || []);
      setIntegrations(integrationsRes.data?.integrations || []);
      setCollegeHealth(collegesRes.data?.colleges || []);
      setTickets(ticketsRes.data?.tickets || []);
      setTicketStats(ticketStatsRes.data?.stats || []);
      setErrorStats(errorsRes.data?.stats || []);
    } catch (err) {

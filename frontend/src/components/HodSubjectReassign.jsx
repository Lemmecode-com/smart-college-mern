 ...
 *
 * Lets a College Admin reassign a current HOD's ACTIVE subjects (and the
 * associated timetable slots / attendance sessions) to other eligible
 * teachers, WITHOUT removing the HOD or touching Department.hod_id,
 * Teacher.status, or User.role.
 *
 * Replacement teachers MUST be from the same DEPARTMENT AND same COURSE
 * as each subject — identical constraint to TeacherDeactivationModal.
 *
 * On success, the admin must separately click "Remove HOD" once the
 * subject count reaches zero. This modal never removes/replaces the HOD.
 */
export default function HodSubjectReassign({
  isOpen,
  onClose,
  departmentId,
  hodTeacherId,
  ...
  useEffect(() => {
    if (!isOpen || !hodTeacherId) return;

    const fetchData = async () => {
      setFetchingData(true);
      try {
        const [reassignmentRes, teachersRes] = await Promise.all([
          api.get(`/teachers/${hodTeacherId}/reassignment-data`),
          api.get(
            `/teachers/available-for-reassignment?excludeTeacherId=${hodTeacherId}`,
          ),
        ]);
        ...
        toast.error("Failed to load HOD reassignment data");
        console.error(err);
      } finally {
   ...
   * Constraint: Same DEPARTMENT + Same COURSE (mirrors the backend
   * reassignTeacherResources service, so the UI never offers a choice
  ...
  /**
   * Subjects that would be submitted without a valid target — either no
   * eligible teacher exists at all for that subject, or neither the
   * per-subject override nor the default teacher is actually eligible.
   */
  const getUnresolvedSubjects = () => {
    if (!reassignmentData?.subjects) return [];

    return reassignmentData.subjects.filter((subject) => {
      const eligible = getEligibleTeachers(subject);
      if (eligible.length === 0) return true;

      const chosen = subjectToTeacherMap[subject._id] || defaultTeacherId;
      if (!chosen) return true;

      return !eligible.some((t) => t._id === chosen);
    });
  };

  const handleSubmit = async () => {
    if (!defaultTeacherId) {
      toast.error("Please select a default target teacher for reassignment");
      return;
    }

    const unresolved = getUnresolvedSubjects();
    if (unresolved.length > 0) {
      toast.error(
        `No eligible teacher assigned for: ${unresolved
          .map((s) => s.name)
          .join(", ")}. Choose a teacher from the same department & course.`,
        { autoClose: 6000 },
      );
      return;
    }
    ...
    setLoading(true);
    try {
      await api.put(`/departments/${departmentId}/hod/reassign-subjects`, {
        defaultTeacherId,
      ...
      toast.success(
        hodName
          ? `Subjects reassigned successfully. "${hodName}" remains HOD.`
          : "Subjects reassigned successfully.",
        {
          position: "top-right",
          autoClose: 4000,
        },
      ...
      const errorMsg =
        ...
        "Failed to reassign subjects. Please try again.";
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
          className="hod-reassignment-modal-overlay"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          onClick={onClose}
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <FaExchangeAlt
                  style={{ width: "36px", height: "36px", color: "#0369a1" }}
                />
              </motion.div>
              <h3
                style={{
              ...
              >
                {hodName ? `${hodName} remains HOD. ` : ""}
                Move active subjects to other teachers — the HOD is not removed.
              </p>
            </div>
            ...
            <div style={{ padding: "1.5rem 2rem", overflowY: "auto", flex: 1 }}>
              {fetchingData ? (
                ...
                >
                  <FaCheckCircle style={{ fontSize: "2rem", marginBottom: "0.75rem" }} />
                  <p style={{ margin: 0, fontWeight: 600 }}>
                    This HOD has no active subjects to reassign.
                  </p>
                  <p
                    style={{
                      margin: "0.5rem 0 0",
                  ...
                  >
                    You can close this dialog and proceed with Remove HOD.
                  </p>
                </div>
              ) : (
                <>
                  ...
                  {hodName && (
                    ...
                    >
                      <FaUserTie style={{ color: "#0369a1" }} />
                      Current HOD: <strong>{hodName}</strong>
                      <span style={{ color: "#64748b", fontWeight: 400 }}>
                        (stays HOD after reassignment)
                      </span>
                    </div>
                  )}
                  ...
                  >
                    ...
                    >
                      <FaUserCheck style={{ color: "#0369a1" }} /> Default
                      Target Teacher <span style={{ color: "#dc3545" }}>*</span>
                    </label>
                    <p
                      style={{
                  ...
                  {reassignmentData?.subjects?.length > 0 && (
                    <div>
                      ...
                      >
                        ...
                        >
                          <FaBookOpen style={{ color: "#0369a1" }} /> Subject
                          Reassignment
                  ...
                  style={{
                    ...
                    background:
                      loading || fetchingData || !defaultTeacherId
                        ? "#94a3b8"
                        : "linear-gradient(135deg, #1a4b6d 0%, #123449 100%)",
                    color: "white",
                    fontWeight: 600,
                    fontSize: "1rem",
                    cursor:

...
exports.deleteTimetableSlot = async (req, res, next) => {
  try {
    ...
    if (!department) {
      throw new AppError("Access denied: Only HOD can delete timetable slots", 403, "HOD_ONLY");
    }
...
const Subject = require("../models/subject.model");
const AppError = require("../utils/AppError");
const { convertTo24Hour, convertTo12Hour } = require("../utils/time.utils");
...
exports.addSlot = async (req, res, next) => {
  try {
    ...
    // ✅ Get semester from timetable if not provided in request
    const semesterToUse = semester || timetable.semester || 1;
    ...
    const timeConflict = await TimetableSlot.findOne({
      ...
      $expr: {
        $and: [
          { $lt: ["$startTime", endTime24] },
          { $gt: ["$endTime", startTime24] },
        ],
      },
    });
    ...
    const slot = await TimetableSlot.create({
      ...
      subject_id,
      teacher_id,
      semester: semesterToUse, // ✅ Use semester from timetable if not provided
      day,

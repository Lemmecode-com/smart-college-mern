...
describe("STU-TC-008 — Admin Approve Student", () => {
  ...
  it("PENDING student without division can now be approved (division moved to enrollment)", async () => {
    const college = await createCollege({ code: "APR", name: "Approve College" });
    ...
    const student = await createStudent({
      ...
      department_id: department._id,
      course_id: course._id,
      fullName: "No Division Student",
      email: "student.nodivision@test.com",
      status: "PENDING",
      admissionYear: 2025,
      currentSemester: 1,
      division: null,
    ...
    expect(updatedStudent.status).toBe("APPROVED");
    expect(updatedStudent.enrollmentNumber).toBe(res.body.student.enrollmentNumber);
  });

  it("APPROVED student without division cannot be enrolled (division required at enrollment)", async () => {
    const college = await createCollege({ code: "APR2", name: "Approve College 2" });

    const admin = await createUser({
      email: "admin@approve2.test.com",
      password: "Test@123",
      role: "COLLEGE_ADMIN",
      college_id: college._id,
      isActive: true,
    ...
    const student = await createStudent({
      ...
      department_id: department._id,
      course_id: course._id,
      fullName: "Enroll No Division Student",
      email: "student.enroll.nodivision@test.com",
      status: "APPROVED",
      admissionYear: 2025,
      currentSemester: 1,
      division: null,
      enrollmentNumber: "APR2-CSE2025-0001",
    });
    ...
    const res = await agent
      .put(`/api/students/${student._id}/confirm-enrollment`)
      .expect(400);

    expect(res.body.error.code).toBe("DIVISION_NOT_ASSIGNED");
